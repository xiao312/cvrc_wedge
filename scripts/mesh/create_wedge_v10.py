...
# Case directory (parent of scripts/mesh/ or scripts/)
if os.path.basename(SCRIPT_DIR) == 'mesh':
...
MSH_FILE = os.path.join(OUTPUT_DIR, "cvrc_wedge.msh")
...
def create_wedge():
    ...
    # Note: smoothing is handled by MESH_SMOOTHING_PASSES in mesh options
    # gmsh.model.mesh.smooth() is not available in all Gmsh versions
    ...
    try:
        ...
        total_cells = sum(len(elem) for elem in elements[1]) if elements[1] else 0
    except:
        total_cells = 0
