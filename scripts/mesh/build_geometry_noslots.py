...
P = {
    ...
    "slots_y": [],  # No oxidizer slots in this version
...
# y-ranges for transfinite settings (no slots — just 2 bands)
ny_ox_passage = 38  # nodes across oxidizer passage (y=3.0 to 10.235)
ny_map = {
    (EPSILON, 3.0): P["ny_0_3"],
    (3.0, 10.235): ny_ox_passage,
}

# 1) Inlet blocks (x=-160 to -146): 2 bands
for y0, y1 in [(EPSILON, 3.0), (3.0, 10.235)]:
    add_surface(
...
# 2) Slot strip blocks (x=-146 to -128): 2 bands, no slot walls
for y0, y1 in [(EPSILON, 3.0), (3.0, 10.235)]:
    add_surface(
        [(-146.0, y0), (-128.0, y0), (-128.0, y1), (-146.0, y1)],
        f"passage_{y0}_{y1}",
        transfinite_counts=(P["nx_slot_strip"] + P["nx_right_local"], ny_map[(y0, y1)]),
    )
...
# Match segmentation at x=-128 (2 bands: axis + passage)
...
wall_lines = sorted(set([
    # Left wall at x=-160: axis to oxidizer passage
    abs(ln(pt(-160.0, EPSILON), pt(-160.0, 3.0))),
    abs(ln(pt(-160.0, 3.0), pt(-160.0, 10.235))),
    # Top of inlet region
    abs(ln(pt(-160.0, 10.235), pt(-146.0, 10.235))),
    abs(ln(pt(-146.0, 10.235), pt(-128.0, 10.235))),
...
axis_lines = sorted(set([
    abs(ln(pt(-146.0, EPSILON), pt(-128.0, EPSILON))),
...
# No oxidizer slots in this version — slot lines become wall
slot_phys = []
